...
export async function generateMetadata({ params }: ToolPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: tool.detailHref,
    },
    openGraph: {
      title: `${tool.name} | ${siteConfig.name}`,
      description: tool.description,
      url: `${siteConfig.url}${tool.detailHref}`,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 900,
          alt: `${tool.name} en ${siteConfig.name}`,
        },
      ],
    },
    twitter: {
...
export default async function ToolDetailPage({ params }: ToolPageProps) {
  ...
  const relatedPosts = getRelatedPostsForTool(tool);
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-14">
      <section className="blog-reveal surface-card relative overflow-hidden rounded-4xl px-5 py-6 sm:px-8 sm:py-8 lg:px-12 lg:py-12">
        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <span className="hero-chip rounded-full px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-(--accent-strong)">{tool.category}</span>
            <span className="hero-chip rounded-full px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-(--accent-strong)">Actualizada {new Date(tool.updatedAt).toLocaleDateString("es-DO")}</span>
          </div>
          <h1 className="mt-4 max-w-4xl text-3xl font-semibold leading-tight text-(--ink) sm:text-5xl lg:text-6xl">{tool.name}</h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-(--muted) sm:text-xl sm:leading-8">{tool.description}</p>
          ...
          <p className="mt-4 max-w-3xl text-sm leading-6 text-(--ink)">{tool.primaryOutcome}</p>

          <div className="action-strip mt-6 rounded-4xl p-4 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-(--accent-strong)">Uso directo</p>
                <h2 className="mt-2 text-xl font-semibold text-(--ink)">Pruebala y decide rapido si encaja en tu flujo.</h2>
              </div>
      ...
      <section className="blog-reveal blog-reveal-delay-1 mt-8 surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="section-label text-xs font-semibold uppercase">Herramienta en uso</p>
            <h2 className="mt-3 text-3xl font-semibold text-(--ink) sm:text-4xl">Pruebala aqui antes de decidir si se queda en tu flujo.</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="hero-chip rounded-full px-3 py-1.5 text-xs font-medium text-(--ink)">{tool.intentLabel}</span>
            <span className="hero-chip rounded-full px-3 py-1.5 text-xs font-medium text-(--ink)">{tool.decisionStage}</span>
          </div>
        </div>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-(--muted)">{tool.primaryOutcome}. La interaccion es local y esta presentada de forma mas compacta para que la prueba no se sienta pesada.</p>
        <div className="mt-6">
      ...
      <section className="mt-8 grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:gap-6">
        <div className="space-y-5">
          <section className="surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
            <p className="section-label text-xs font-semibold uppercase">Beneficios clave</p>
          ...
          <section className="surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
            <p className="section-label text-xs font-semibold uppercase">Como usarla</p>
            <h2 className="mt-3 text-2xl font-semibold text-(--ink) sm:text-3xl">Ruta simple para empezar sin friccion.</h2>
            <div className="mt-5 grid gap-3">
              {tool.quickSteps.map((step, index) => (
                <div key={step} className="rounded-3xl border border-(--line) bg-white/55 px-4 py-4">
                  <p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-(--highlight)">Paso {index + 1}</p>
          ...
          <section className="surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
            <p className="section-label text-xs font-semibold uppercase">Casos de uso</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <aside className="space-y-5">
          <section className="surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
            ...
            <h2 className="mt-3 text-2xl font-semibold text-(--ink)">Que conviene saber antes de usarla</h2>
            <div className="mt-5 grid gap-3">
            ...
            <div className="mt-5 border-t border-(--line) pt-5">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-(--accent-strong)">Mejor para</p>
          ...
          {relatedPosts.length > 0 ? (
            <section className="blog-reveal blog-reveal-delay-2 surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
              <p className="section-label text-xs font-semibold uppercase">Guias relacionadas</p>
              <div className="mt-5 grid gap-3">
                {relatedPosts.map((post) => (
                  <TrackedLink key={post.slug} href={`/blog/${post.slug}`} eventName="related_post_clicked" payload={{ tool: tool.slug, post: post.slug }} className="blog-card-premium rounded-3xl border border-(--line) bg-white/60 px-4 py-4 hover:border-(--accent)">
                    ...
                    <p className="mt-4 text-sm font-semibold text-(--accent-strong)">Leer guia</p>
                  </TrackedLink>
                ))}
              </div>
            </section>
      ...
      {similarTools.length > 0 ? (
        <section className="mt-8 surface-card rounded-4xl px-5 py-6 sm:px-8 sm:py-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="section-label text-xs font-semibold uppercase">Herramientas relacionadas</p>
              <h2 className="mt-3 text-3xl font-semibold text-(--ink)">Si esta no encaja, prueba otra ruta cercana.</h2>
            </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {similarTools.map((entry) => (
              <article key={entry.slug} className="blog-card-premium rounded-[1.75rem] border border-(--line) bg-white/55 p-5 hover:border-(--accent) sm:p-6">
                ...
                <p className="mt-3 text-sm leading-7 text-(--muted)">{getToolRelationReason(tool, entry)}</p>
